...
export class ChannelCreateListener extends Listener<typeof Events.ChannelCreate> {
  ...
  public async run(channel: GuildChannel) {
    if (!channel.guild) {
      this.container.logger.debug({ channelId: channel.id }, 'Channel create: Channel not in guild, returning');
      return;
    }
    ...
    this.container.logger.debug(
      ...
      'Channel create event triggered'
    );
    ...
    // Try to fetch the user who created the channel from audit logs
    let actor = null;
    try {
      // Try multiple times to fetch audit logs since they might not be immediately available
      actor = await this.fetchActorFromAuditLogs(channel.guild, channel.id, 10, 5); // Channel create = 10, try 5 times
    } catch (error) {
      ...
      this.container.logger.debug(
        ...
        'Error fetching audit logs for channel creation'
      );
    }
    ...
    const description = [
      `### ${getEmoji('channel')} Channel Created`,
      `> **Name:** ${identifier}`,
      `> **Type:** ${channelType}`,
      `> **Created By:** <@${actor?.id || 'unknown'}>${actor?.bot ? ' (Bot)' : ''}`,
      `> **Channel:** <#${channel.id}>`,
      `> **ID:** \`${channel.id}\``
    ].join('\n');
    ...
    this.container.logger.debug(
      ...
      'Prepared channel create log description'
    );
    ...
    try {
      ...
      this.container.logger.error(
        ...
        'Failed to fetch guild config for channel create log'
      );
      return;
    }
    ...
    this.container.logger.debug(
      ...
      'About to send channel create log'
    );

    // Send the channel log, handling any errors gracefully
    try {
      await this.container.logging.sendChannelCreateLog(channel.guild, description, {
        actor: actor,
