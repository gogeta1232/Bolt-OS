...
export class UnifiedReactionRemoveListener extends Listener<typeof Events.MessageReactionRemove> {
  public async run(reaction: MessageReaction | PartialMessageReaction, user: User | PartialUser) {
    try {
      ...
      await reactionHandlerService.handleReactionRemove(reaction as MessageReaction, user as User);
    } catch (error) {
      this.container.logger.error({ err: error }, 'Error handling unified reaction remove');
    }
  }
}
