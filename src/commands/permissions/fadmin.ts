...
@ApplyOptions<Command.Options>({
  name: 'fadmin',
  description: 'Grant or revoke the Bolt admin key on a role (owner-only).',
  aliases: ['setadminrole'],
  runIn: ['GUILD_ANY'],
  requiredClientPermissions: ['SendMessages']
})
export class FadminCommand extends AdminCommand {
  ...
  public override async messageRun(message: Message, args: Args) {
    ...
    if (!role) {
      await channel.send(createDangerEmbed('Usage: fadmin <@role>', false));
      return;
    }
  ...
  private async applyRole(guild: Guild, roleId: string) {
    ...
    return `${getEmoji('target')} **Admin Key ${isAdd ? 'Granted' : 'Revoked'}**\n\nRole: ${role?.toString() ?? roleName}\nTotal admin roles: ${count}\n\n${isAdd ? `${getEmoji('success')} Holders get full Bolt access and can grant the mod key with \`fmod\`.` : `${getEmoji('danger')} Holders lose all Bolt access.`}`;
  }
}
